...
import { unifiedDraftsService } from '@/services/unifiedDraftsService';
...
export function UnifiedDraftsFeed({ 
  ...
  const loadRealDrafts = async () => {
    try {
      const realDrafts = await unifiedDraftsService.getAllDrafts();
      const formattedDrafts: Draft[] = realDrafts.map(d => ({
        id: d.id,
        type: d.type as 'email' | 'calendar' | 'task' | 'note',
        title: d.title,
        content: d.preview,
        ...
        source: d.source === 'ladder' ? 'auto_write' as const : 'manual' as const,
        metadata: {
          reasoning: `Generated by ${d.source} service`,
          undoId: `undo-${d.id}`
        }
      }));
      
      // Add sample drafts for demo if no real drafts
      ...
      console.error('Failed to load real drafts, using samples', error);
      loadSampleDrafts();
    }
  };

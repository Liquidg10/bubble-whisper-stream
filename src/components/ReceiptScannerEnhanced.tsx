...
export const ReceiptScannerEnhanced: React.FC<ReceiptScannerEnhancedProps> = ({
  ...
  const saveFinanceData = async (data: FinanceMetadata, autoTag: boolean = false) => {
    try {
      ...
      // Assign to budget envelope if selected
      if (selectedEnvelope && isFeatureEnabled('budget')) {
        await budgetService.assignReceiptToEnvelope(bubble.id, selectedEnvelope, data);

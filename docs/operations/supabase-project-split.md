...
- the private `photos` bucket and its owner-scoped policies
...
2. Run exact row counts. Stop if any durable Mind Manual table contains user
   data that has not been explicitly migrated; transient OAuth state is not
   copied.
